...
Cubit 是一款以 C#/.NET 与 Vulkan 构建的通用游戏引擎，采用场景树、服务端渲染资源管理、ECS 与任务系统协同的架构。引擎核心保持领域无关，体素、物理、音频、动画、脚本和内容导入等能力以独立官方插件提供；内置编辑器、MCP 自动化接口及桌面/Android 运行宿主共同构成完整开发工作流。具体游戏项目通过场景和项目清单组合引擎能力，项目玩法与资产始终独立于引擎仓库。

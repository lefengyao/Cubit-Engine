...
/// <summary>
/// SDL 音频设备的小型可替换入口。实现必须在 <see cref="QueueAudio"/> 返回前消费输入样本。
/// </summary>
...
/// <summary>
/// Windows SDL2 队列音频输出后端。每个实例只持有一个设备，不创建额外工作线程或全局服务。
/// </summary>
public sealed class SdlAudioOutputBackend : IAudioOutputBackend
{
    ...
    public const string BackendId = "cubit.audio.windows.sdl2";
    ...
    /// <summary>用可替换 SDL 入口创建后端，供宿主集成与离线契约验证使用。</summary>
        ...
        int maximumQueuedBytes = DefaultMaximumQueuedBytes)
    {
        ...
        try
        {
            ...
            if (_deviceId == 0)
            {
                ThrowOpenFailure("无法打开 SDL 音频设备");
            }
            ...
            if (obtainedFormat != SdlAudioDeviceFormat.Fixed)
            {
                ...
                throw new AudioBackendException(
                    "audio.backend.format_mismatch",
                    "SDL 音频设备未协商到 Cubit Audio 1.0 固定格式");
            }
        ...
        catch (Exception exception)
        {
            ...
            Report(
                ...
                $"SDL 音频设备初始化失败: {exception.GetType().Name}");
            _adapter.Dispose();
            throw new AudioBackendException("audio.backend.open_failed", "SDL 音频设备初始化失败");
        }
    }
        ...
        lock (_sync)
        {
            ...
            if (!_adapter.QueueAudio(_deviceId, samples))
            {
                Report(
                    ...
                    $"SDL 音频队列提交失败: {_adapter.GetError()}");
            }
        }
    }
    ...
    private void ReportClosedOnce()
    {
        ...
        Report("audio.backend.closed", "SDL 音频后端已经关闭，忽略提交");
    }
    ...
    private void ReportQueueFullOnce()
    {
        ...
        Report("audio.backend.queue_full", "SDL 音频队列已达到固定上限，丢弃当前 PCM 块");
    }
